...
public abstract class Enemy : MonoBehaviour
{
    ...
    protected virtual void CallStateMachine()
    {
        switch (currentState)
        {
            ...
            case States.STATE_PATROLLING:

                PatrolMovement();

                if (spotPlayer)
                {
                    currentState = States.STATE_CHASING;
                }

            break;
            case States.STATE_CHASING:
                Debug.Log("Chasing");
                ChasePlayer();

            break;
            case States.STATE_SEARCHING:
                Debug.Log("Searching");
                searchPlayer();
            break;

            case States.STATE_RETURNING:
    ...
    void CallEnemies()
    {
        ...
        {
            if (results[i].GetComponent<Enemy>().currentState != States.STATE_CHASING)
            {
...

    {
...

        {
            ...
            currentState = States.STATE_RETURNING;
